...
func main() {
	...
	http.HandleFunc("/game-output", func(w http.ResponseWriter, r *http.Request) {
	...
	client, err := sarama.NewConsumerGroup([]string{"188.166.225.204:9092"}, "test-game", config)
	if err != nil {
	...
	go func() {
		...
		for {
			...
			if err := client.Consume(ctx, []string{"game-color"}, &consumer); err != nil {
...
func (consumer *Consumer) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	...
	for message := range claim.Messages() {
		...
		for _, conn := range consumer.connMap {
			if err := conn.WriteMessage(msgType, message.Value); err != nil {
				continue
			}
		}
